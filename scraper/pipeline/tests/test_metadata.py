...
def test_maybe_write_run_metadata_swallows_write_failures_and_returns_metadata_path(tmp_path: Path, monkeypatch) -> None:
    model_root = tmp_path / "work" / "233541"
    artifacts = RunArtifacts(model_root=model_root, scrape_dir=model_root / "scrape")
    ...
    assert returned_path == model_root / "render.run.json"
