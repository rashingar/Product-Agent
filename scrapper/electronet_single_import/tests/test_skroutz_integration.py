...
def test_validate_input_rejects_skroutz_sections() -> None:
    args = argparse.Namespace(model="341490", url=SAMPLES["341490"]["url"], photos=1, sections=1, skroutz_status=0, boxnow=0, price="19", out="out")
    try:
        validate_input(args)
    except ValueError as exc:
        assert str(exc) == "Skroutz v1 supports only sections=0"
    else:
